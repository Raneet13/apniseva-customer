...
class _SearchFieldState extends State<SearchField> {
  ...
  Future<Iterable<SearchDtl>> _suggestionCallBack(suggestion) async {
    if (suggestion.isEmpty ||
        searchController.dashDataModel.value.messages?.status?.searchDtl == null) {
      return const Iterable<SearchDtl>.empty();
    }
    return searchController.dashDataModel.value.messages!.status!.searchDtl!
    ...
    return Card(
      child: TypeAheadField<SearchDtl>(
        ...
        suggestionsCallback: (pattern) {
          if (pattern.isEmpty ||
              searchController.dashDataModel.value.messages?.status?.searchDtl ==
                  null) {
            return [];
          }
          var filteredCities = searchController
              .dashDataModel.value.messages!.status!.searchDtl!
              .where((SearchDtl serviceName) =>
                  serviceName.serviceName
                      ?.toLowerCase()
                      .toString()
